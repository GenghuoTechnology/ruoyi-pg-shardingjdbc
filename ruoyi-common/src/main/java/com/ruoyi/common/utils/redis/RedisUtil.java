...
public class RedisUtil {
    ...
    public Set<String> getKeys(String matchKey){
...
logger.error(e.getMessage());
     ...
     * @version: 1.0
     */
    public Set<String>  getBySuffix(String suffix) {
        Set<String> keys=redisTemplate.keys("*"+suffix);
        return  keys;
    }
    /**
     ...
     * @version: 1.0
     */
    public long deleteByPreAndKeys(String suffix, List<Long> list) {
        StringBuilder stringBuilder=new StringBuilder(suffix);
        for(Long item: list) {
            stringBuilder.append("*"+item);
        }
        stringBuilder.append("*");
        Set<String> keys=redisTemplate.keys(stringBuilder.toString());
        return  redisTemplate.delete(keys);
    }
    public RedisAtomicInteger getRedisCounter(String key,int minutes) {
        RedisAtomicInteger counter =
                new RedisAtomicInteger(key, stringRedisTemplate.getConnectionFactory());
        if (counter.get() == 0) {
            // 设置过期时间，10分钟
            counter.expire(minutes, TimeUnit.MINUTES);
        }
        return counter;
    }
    public RedisAtomicInteger getRedisCounter(String key) {
        RedisAtomicInteger counter =
                new RedisAtomicInteger(key, stringRedisTemplate.getConnectionFactory());
        return counter;
    }
    ...
    /**
     * @description: 最右边移除list中的值并返回移除对象
     * @data:[key] key
